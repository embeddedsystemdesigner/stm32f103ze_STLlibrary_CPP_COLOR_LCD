...
void wait_ms(unsigned int d)
{	/* for 120 MHz @ Level0 Opt */
	unsigned int i;
	i=d;while(i){i--;wait_us(998);}
}
...
{ 										  
  ...
  GPIO_Init(GPIOG, &GPIO_InitStructure);
    ...
    writeTiming.FSMC_AddressHoldTime = 0x02;	
...
wait_ms(50);		 
	...
	{	   
    	...
    	LCD_WriteReg(0x0010,0x0000);  
    	LCD_WriteReg(0x0011,0x6078); //0x6058
...
void show_image(int x,int y,int sizex, int sizey,const unsigned short *image)
{
...
int count=0;
	...
	sizex+=xx;
  	...
  	LCD_Fast_DrawPoint(xx,y,image[count]);
...
__inline void tft_PutPixel(uint16_t x, uint16_t y, uint32_t brush)
{
	
	
	
	#ifdef PORTRAIT
  LCD_WriteReg(0x0050,x);
  LCD_WriteReg(0x0051,x);
  LCD_WriteReg(0x0052,y);
  LCD_WriteReg(0x0053,y);
  LCD_WriteReg(0x0020,x);
  LCD_WriteReg(0x0021,y);
#endif
#ifdef LANDSCAPE
  LCD_WriteReg(0x0052,x);
  LCD_WriteReg(0x0053,x);
  LCD_WriteReg(0x0050,y);
  LCD_WriteReg(0x0051,y);
...
void tft_showimage(int x,int y,int sizex, int sizey,const unsigned short *image)
{
...
void LCD_fastimage(const unsigned short *image)
{
	u32 index=0;      
	u32 totalpoint=lcddev.width;
	totalpoint*=lcddev.height; 	
